...
- 夜間順序是**全遊戲一份**：守衛 → 禁言 → 攝夢 → 狼人 → 女巫 → 預言家（`NIGHT_ACTION_ORDER`）。
  含狼美人的版型另外把**魅惑插在狼刀之前**（2026-09-26 照來源站法官順序調整，見 §5.5）。
...
| 8 | 白狼王騎士 | 預言家、女巫、獵人、騎士、平民×4 | 狼人×3、白狼王 | **已存在** `official-12-white-wolf-knight` | A | — | 這一列原本只是照抄網站、沒對過程式：程式當時是**守衛**，已依網站改成**獵人**（版型以網站為準）。 |
...
- **「狼美人被騎士決鬥出局時不發動魅惑」**：兩個來源頁都明講。**2026-09-26 使用者裁定**：
  除騎士決鬥外**一律發動殉情**，包含夜間出局（被女巫毒死、被夢帶走都算）——是「出局時」
  而不是「白天出局時」（`charm.ts` 的 `triggersCharmRevenge` 即 `cause !== "duel"`；
  玩家看到的 `roleReveal.roles.wolfBeauty` 三語系說明也已經是這個寫法）。
  本作**早已實作**：`charm.ts` 的 `triggersCharmRevenge` 就是 `cause !== "duel"`，
  `knight-duel.ts` 的註解也明列狼美人殉情不可發動，`charm.test.ts` 有 3 條斷言。
  這次是逐條查證，沒有改動程式。
- **法官夜間順序把「魅惑」排在狼刀之前**（魅惑 → 守衛／狼刀 → 女巫 → 預言家）：**2026-09-26 已照此調整**
  （`NIGHT_ACTION_ORDER`：守衛 → 禁言 → 攝夢 → 魔術師 → **魅惑 → 狼人** → 女巫 → 預言家）。
  兩者結算等價（魅惑只取決於狼美人自己是否出局，與當晚刀口無關），所以這次調序只改變玩家看到的
  行動順序，不改變任何結算結果。
...
**實機測試抓到的第二個坑（同一天，玩到夜間結算才現形）**：`nightHistory[1].magicianSwap` 是 `null`。
原因是**真正的結算**（`useSpecialEvents`）自己手寫了一份夜史欄位清單，沒跟著新增換位；
狼美人先前（記錄裡「魅惑: 無」）就是同一個病灶。這次改成共用清單 `rules/night-record.ts`
＋守門測試（掃原始碼：任何 `nightActions.<欄位> =` 的寫入端都必須在清單裡或被明確排除）。
守門一上線就抓到**第二個既有缺口**：禁言長老的目標從來沒進過夜史（舊手寫清單就漏了），
連帶補上型別與 DevConsole 的顯示列。
...
## 版型排列順序（UI 下拉／開發者分頁／教學頁共用）

12 人版的顯示順序照參考目錄 <https://werewolves.games/variants/>（該站按學習難度／機制密度排：
預女獵白 → 預女獵禁 → 狼王守衛 → 狼王攝夢 → 狼王魔術師 → 白狼王騎士 → 狼美騎士 → 魔鬼騎士 →
四狼八獵）。我們只收錄其中一部分，照它的**相對**順序排；它沒收錄的兩個插在性質相近處：

- `official-12-classic`（經典）固定第一個——`getDefaultBoard` 取 `boards[0]`，這就是 12 人局的
  **預設版型**，動到它等於改掉開局預設；
- `official-12-seer-witch-guard-idiot`（預女守白）跟在預女獵白／預女獵禁之後。

順序由 `src/lib/rules/boards.ts` 的 `OFFICIAL_BOARDS` 陣列決定（`getBoardsByPlayerCount` 只依人數
filter，不再排序），`boards.test.ts` 有一條清單把它釘住；改順序要一起更新那條清單與程式碼註解。
