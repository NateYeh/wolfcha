...
- 自爆者死亡、**只出自爆宣言不給遺言**；被白狼王帶走的人無遺言（現行）。
...
> 註：若 26 個版型中含未實作角色（狼美人、隱狼、騎士、石像鬼…），Phase 3 會被迫依賴
> Phase 4，因此**先鎖定「只用現有 8 角色」的版型做第一波**。
...
1. 普通狼在**白天／PK 發言**自爆：是否也「直接天黑＋跳過放逐投票＋不帶人」？
   （草案採「是」）
2. 自爆者是否給遺言？（草案：只出自爆宣言，不給遺言）
3. 「天亮後繼續競選」＝接續**未發言**的候選人（已發言者不重複），之後競選投票照常進行？
4. 第一次自爆若是**白狼王**（已吞徽）→ 是否還需第二次自爆才算競選結束？
   （草案：不用，白狼王那一次即結束競選）
5. 26 個官方版型是否**全用現有 8 角色**？若要新角色，請給清單與優先序。
