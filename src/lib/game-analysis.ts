...
export async function generateGameAnalysis(
  ...
  const personalStats: PersonalStats = {
    ...
    gender: humanPlayer.agentProfile?.persona?.gender,
    avatarStyle: humanPlayer.avatarStyle,
...
2-4. 评选按三档顺序：①关键转折优先——胜负手是谁创造的；终结性动作（最后一枪、压哨一票）若只是执行他人提供的资讯（按查杀、点名、药帐、警徽流定的目标），创造者排在执行者前面，reason 里写清这条因果链。②实际贡献其次——操作要实质推进胜利（票中狼、查中狼、毒中狼、守住刀口、带崩对方判断），话多、跳得响但无收益不算贡献。③综合表现是下限——整场都在送（投错票、误带好人、喂狼信息）的玩家，单次再亮眼也不给票。
  ...
  const normalizedReviews: PlayerReview[] = result.reviews.map((review): PlayerReview => {
    ...
    if (reviewer) {
      ...
      return {
        ...
        gender: reviewer.agentProfile?.persona?.gender,
        avatarStyle: reviewer.avatarStyle,
        relation,
        role: reviewer.role,
      };
    }
  ...
  return {
    awards: {
      mvp: {
        ...
        gender: mvpPlayer?.agentProfile?.persona?.gender,
        avatarStyle: mvpPlayer?.avatarStyle,
      ...
      svp: {
        ...
        gender: svpPlayer?.agentProfile?.persona?.gender,
        avatarStyle: svpPlayer?.avatarStyle,
