...
test("步驟表正好覆蓋權威順序（多一個或少一個都會紅）", () => {
  assert.deepEqual([...NIGHT_ACTION_ORDER], ["NIGHT_GUARD_ACTION", "NIGHT_MUTE_ACTION", "NIGHT_DREAM_ACTION", "NIGHT_MAGICIAN_ACTION", "NIGHT_WOLF_BEAUTY_ACTION", "NIGHT_WOLF_ACTION", "NIGHT_WITCH_ACTION", "NIGHT_SEER_ACTION"]);
...
test("補齊查詢：after 只看它之後的步驟（跳階時補齊用）", () => {
  ...
  assert.deepEqual(
    ...
    ["NIGHT_MAGICIAN_ACTION", "NIGHT_WOLF_BEAUTY_ACTION", "NIGHT_WOLF_ACTION", "NIGHT_WITCH_ACTION", "NIGHT_SEER_ACTION"]
  );
  assert.deepEqual(
